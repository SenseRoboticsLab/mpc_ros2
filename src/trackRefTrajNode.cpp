...
MPCNode::MPCNode()
{
    ...
    _min_distance = 0.0;
    _dis_pub = _nh.advertise<std_msgs::Float64>("/min_track_distance", 1);
}
...
{
    _odom = *odomMsg;
    // ROS_INFO_STREAM("odom update to: [x y z qx qy qz qw]"<<_odom.pose.pose.position.x<<" "<<_odom.pose.pose.position.y<<" "<<_odom.pose.pose.position.z<<" "<<_odom.pose.pose.orientation.x<<" "<<_odom.pose.pose.orientation.y<<" "<<_odom.pose.pose.orientation.z<<" "<<_odom.pose.pose.orientation.w);
...
void MPCNode::desiredPathCB(const nav_msgs::Path::ConstPtr& totalPathMsg)
{
    ...
    try
    {
        ...
        // int N = std::min(static_cast<unsigned int>(min_idx + totalPathMsg->poses.size()*0.1),
        //                  static_cast<unsigned int>(totalPathMsg->poses.size())) ; // Number of waypoints
        ...
        for(int i = 0; i < N; i++)
        {
            ...
            if(min_val > sqrt(dx*dx + dy*dy))
            {
                ...
                min_idx = i;
                // ROS_INFO_STREAM("min idx update to: "<<min_idx<<" min distance: "<<min_val);
            }
        }
...
{
    // ROS_INFO_STREAM("goal_received: "<<_goal_received<<" goal_reached: "<<
    //     _goal_reached<<" path_computed: "<<_path_computed);
    if(_goal_received && !_goal_reached && _path_computed ) //received goal & goal not reached
    {
        ...
        // Eigen::Matrix3d R_cur_goal = q_cur.toRotationMatrix().inverse() * q_goal.toRotationMatrix();
        Eigen::AngleAxisd r_axis(q_cur);
        double yaw_cur = (r_axis.angle() * r_axis.axis()).z();
        // r_axis = Eigen::AngleAxisd(q_goal);
        // double yaw_goal = (r_axis.angle() * r_axis.axis()).z();
        // ROS_INFO_STREAM("yaw cur: "<<yaw_cur<<" yaw goal: "<<yaw_goal);
        double yaw_dis = yaw_goal - yaw_cur;
        // ROS_INFO_STREAM("rotation axis: "<<(r_axis.angle() * r_axis.axis()).transpose());
        if(yaw_dis>M_PI){
            yaw_dis -= 2*M_PI;
        }
        ...
        // ROS_INFO_STREAM("yaw dis: "<<yaw_dis);
    ...
    msg.data = _min_distance;  // Replace with your double value
